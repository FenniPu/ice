...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zDEV spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zDEV without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zDEV spend tx %s already in block %d",
			...
			// Check that zDEV mints are not already known
			if (tx.IsZerocoinMint()) {
				for (auto& out : tx.vout) {
					if (!out.IsZerocoinMint())
						continue;
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight < 225650 && nHeight >= Params().LAST_POW_BLOCK()) {
		nSubsidy = 20 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZDEVStake)
{
	int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		 ...
		 * note we only undo zerocoin databasing in the following statement, value to and from Deviant
...
{
	RenameThread("deviant-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZDEVMinted()
{
...
void RecalculateZDEVSpent()
{
	...
	while (true) {
		...
		//Remove spends from zDEV supply
...
bool UpdateZDEVSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		...
		RecalculateDEVSupply(Params().Zerocoin_StartHeight());
	}

	//Track zDEV money supply in the block index
	if (!UpdateZDEVSupply(block, pindex))
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zDevSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
	...
	// If turned on AutoZeromint will automatically convert DEV to zDEV
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	...
	if (CZDevStake* zDEV = dynamic_cast<CZDevStake*>(stake)) {
		CBlockIndex* pindexFrom = zDEV->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zDEV stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zDEV stake does not have required confirmation depth", __func__);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zDEV->GetValue()));
		if (nChecksum200 != zDEV->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zDEV->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZDEV() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zDEV fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zDEV mints and %d zDEV spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		if (pfrom->nVersion != 0) {
			...
			LOCK(cs_main);
			Misbehaving(pfrom->GetId(), 1, 2);
			return false;
		}

		// Deviant: We use certain sporks during IBD, so check to see if they are
		...
		if (state.IsInvalid(nDoS)) {
			...
			if (nDoS > 0)
				Misbehaving(pfrom->GetId(), nDoS, 9);
		}
	}
	...
	else if (strCommand == "headers" && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
	{
		...
		BOOST_FOREACH(const CBlockHeader& header, headers) {
			...
			if (!AcceptBlockHeader((CBlock)header, state, &pindexLast)) {
				...
				if (state.IsInvalid(nDoS)) {
					if (nDoS > 0)
						Misbehaving(pfrom->GetId(), nDoS, 12);
		...
		if (!mapBlockIndex.count(block.hashPrevBlock)) {
			...
			if (!mapBlockIndex.count(block.GetHash())) {
				...
				if (state.IsInvalid(nDoS)) {
					...
					if (nDoS > 0) {
						TRY_LOCK(cs_main, lockMain);
						if (lockMain) Misbehaving(pfrom->GetId(), nDoS, 13);
					}
				}
	...
	// getaddr message mitigates the attack.
		...
		LOCK(cs_main);
		Misbehaving(pfrom->GetId(), 100, 15);
	}
		...
		if (!filter.IsWithinSizeConstraints()) {
			...
			LOCK(cs_main);
			Misbehaving(pfrom->GetId(), 100, 16);
		}
		else {
			LOCK(pfrom->cs_filter);
		...
		if (vData.size() > MAX_SCRIPT_ELEMENT_SIZE) {
			LOCK(cs_main);
			Misbehaving(pfrom->GetId(), 100, 17);
		}
		else {
			LOCK(pfrom->cs_filter);
			if (pfrom->pfilter)
				...
				LOCK(cs_main);
				Misbehaving(pfrom->GetId(), 100, 18);
			}
		}
	}
